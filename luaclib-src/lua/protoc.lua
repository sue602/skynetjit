...
local function register_type(self, lex, tname, typ)
   ...
   -- require("skynet").error("register_type ====",tname,type(tname), typ, type(typ))
...
local function check_field(self, lex, info)
   ...
   if info.type_name then
      local strarray = string.split(info.type_name,".")
      if #strarray == 2 then
		local packagearray = string.split(self.prefix[2],".")
		table.remove(packagearray,#packagearray)
        info.type_name = table.concat(packagearray,".") .. "." .. info.type_name
      end
...
function Parser:compilefile_new(fn)
   if self == Parser then self = Parser.new() end
   local set = do_compile(self, self.parsefile, self, fn)
   return set
end

function Parser:loadfile_loaded(set)
   if self == Parser then self = Parser.new() end
   local ret, pos = pb.load(pb.encode('.google.protobuf.FileDescriptorSet', set))
   if ret then return ret, pos end
   error("load failed at offset "..pos)
end
