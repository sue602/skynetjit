...
// 判断文件是否存在
static int lfs_exist (lua_State *L) {
    const char *path = luaL_checkstring(L, 1);
    lua_pushboolean (L, !access(path,0));
    return 1;
}
...
static int
lfs_time(lua_State *L) {
#if !defined(__APPLE__) || defined(AVAILABLE_MAC_OS_X_VERSION_10_12_AND_LATER)
  struct timespec ti;
  clock_gettime(CLOCK_REALTIME, &ti);
  lua_pushnumber(L, ti.tv_sec + ti.tv_nsec/1000000.0 / 1000.0);
#else
  struct timeval tv;
  gettimeofday(&tv, NULL);
  lua_pushnumber(L, tv.tv_sec + tv.tv_usec / 1000.0 / 1000.0);
#endif
  return 1;
}
