...
int CompressBuffer(unsigned char **dest, const unsigned char *src, unsigned int slen){
    ...
    if (Z_OK != deflateInit2(&stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY))
    {
    ...
    *dest = (unsigned char *) skynet_malloc(dlen);
    if (*dest == NULL) 
    {
        printf("dest is NULL \n");
        return 0;
    }
    stream.next_in = (const Bytef *)src;
...
static int 
UncompressBuffer(unsigned char **dest, const unsigned char *src, unsigned int slen){
    ...
    *dest = (unsigned char *) skynet_malloc(ret + 1);
    if (*dest == NULL) {
        inflateEnd(&stream);
        return 0;
    }
...
static int lz_compress(lua_State *L){
    char * szSrc = (char*) lua_touserdata(L, 1);
    int nSrcLen = luaL_checkinteger(L, 2);
    unsigned char * szZip = { 0 } ;
    unsigned long nZipLen = CompressBuffer(&szZip, szSrc, nSrcLen);
    // printf("lz_compress= %d,nZipLen=%d \n",nSrcLen,nZipLen);
    if (nZipLen > 0){
        lua_pushlightuserdata(L, szZip);
        lua_pushinteger(L,nZipLen);
        return 2;
    }
    lua_pushnil(L);
    return 1;
