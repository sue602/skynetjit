...
usage() {
	...
	common_usage
	echo
	echo "Requires Linux x64, a native GCC/make toolchain and Git. Socket backend: epoll."
}
...
echo "Building Skynet for Linux (epoll)..."
...
if [ "$RUN_TESTS" -eq 1 ]; then
	bash "$ROOT_DIR/scripts/test-runtime.sh" linux "$OUT_DIR"
fi
